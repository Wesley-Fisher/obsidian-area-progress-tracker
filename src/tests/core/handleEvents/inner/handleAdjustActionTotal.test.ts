...
describe("handleAdjustActionTotal", () => {
  ...
  it("clamps totals to action.max when configured", async () => {
    ...
    const config: SystemConfig = {
      ...
      areas: [],
      groups: [],
      actions: [{ id: "walk", name: "Walk", input: { type: "button", step: 1 }, effects: {}, max: 2, placements: [] }],
      records: [],
      requiredActions: {},
      dailyPlan: { actions: {} },
      weeklyPlan: { actions: {} },
  ...
  it("does not clamp when action.max is invalid", async () => {
    ...
    const config: SystemConfig = {
      ...
      groups: [],
      actions: [
        { id: "walk", name: "Walk", input: { type: "button", step: 1 }, effects: {}, max: Number.NaN, placements: [] },
      ],
      records: [],
      requiredActions: {},
      dailyPlan: { actions: {} },
