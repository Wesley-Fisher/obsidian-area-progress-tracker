...
describe("seedFromPreviousDay", () => {
  it("seeds if previous day exists", async () => {
    ...
    const config: SystemConfig = {
      ...
      groups: [{ id: "morning", name: "Morning", columns: [{ id: "default", name: "Default" }] }],
      actions: [{ id: "walk", name: "Walk", input: { type: "button", step: 1 }, effects: { health: 12 }, placements: [], max: 0 }],
      records: [{ id: "weight", name: "Weight", input: { type: "number" }, placements: [{ groupId: "morning", columnId: "default" }] }],
      requiredActions: {},
      dailyPlan: { actions: {} },
      weeklyPlan: { actions: {} },
      stats: { entries: [] },
...
describe("recomputeForwardChain", () => {
  ...
  it("clamps per-day action totals to action.max", async () => {
    ...
    const config: SystemConfig = {
      ...
      groups: [],
      actions: [
        { id: "walk", name: "Walk", input: { type: "button", step: 1 }, effects: { health: 12 }, max: 2, placements: [] },
      ],
      records: [],
      requiredActions: {},
      dailyPlan: { actions: {} },
