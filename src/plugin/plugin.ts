  ...
  private enqueueRefresh(notePath: string): Promise<void> {
    ...
    const next = prev
      ...
      .then(async () => {
        ...
        const uiState = captureUiState(blockEls);
        ...
        for (const [el, refresh] of entries) {
          ...
          restoreUiState(uiState, blockEls);
        }
      ...
      async (source, el, ctx) => {
        ...
        const onUserAction = async (evt: UserEvent): Promise<void> => {
          ...
          if (notePath) {
            ...
            const blocks = el ? [el] : [];
            const uiState = captureUiState(blocks);
            try {
              await refreshSelf?.();
            } finally {
              restoreUiState(uiState, blocks);
            }
          }
        };
...
function captureUiState(blockEls: HTMLElement[]): CapturedUiState {
...
function restoreUiState(state: CapturedUiState, blockEls: HTMLElement[]): void {
